...
const WEAPONS_CONFIG = {
  cross:  { id: "cross",  name: "Mine en croix",    icon: "💣", desc: "5 cases en croix" },
  random: { id: "random", name: "Frappe aléatoire", icon: "🎲", desc: "6 cases au hasard" },
  atomic: { id: "atomic", name: "Bombe atomique",   icon: "☢️", desc: "Cercle ~6×6" },
};
...
function rollNextWeaponDelay() {
  return Math.floor(Math.random() * 6) + 3; // 3 à 8 tours
}
...
function publish(event, data) {
  state.channel.publish(event, { ...data, from: state.pseudo });
}
...
document.getElementById("btn-create").addEventListener("click", async () => {
  ...
  try {
    await connectAbly(pseudo);
  } catch (e) {
    showNotif("Erreur de connexion. Vérifie ta connexion internet. 😬");
    return;
  }
...
// Pré-remplissage du code depuis l'URL
...
function subscribeChannel() {
  ...
  // Host accueille le guest
  ch.subscribe("welcome", (msg) => {
    if (state.role !== "guest") return;
    state.opponentPseudo = msg.data.pseudo;
    startPlacement();
    // Notifier le host de démarrer son placement aussi
    publish("guest-ready-placement", {});
  });

  // Host démarre son placement quand le guest est prêt
  ch.subscribe("guest-ready-placement", () => {
    if (state.role !== "host") return;
    startPlacement();
  });

  // Synchronisation : les deux joueurs ont validé leurs bateaux
  ch.subscribe("placement-done", (msg) => {
    if (msg.data.from === state.pseudo) return;
    state.guestReady = state.role === "host" ? true : state.guestReady;
    state.hostReady = state.role === "guest" ? true : state.hostReady;

    // Côté host : les deux sont prêts → démarre le jeu
    if (state.role === "host" && state.hostReady) {
      publish("game-start", { firstPlayer: state.pseudo });
      startGame(true);
    }
    // Côté guest : attendre game-start
  });
...
document.getElementById("btn-confirm-placement").addEventListener("click", () => {
  ...
  if (state.role === "host") state.hostReady = true;
  if (state.role === "guest") state.guestReady = true;
...
function clearPreview() {
  document.querySelectorAll("#placement-grid .cell").forEach(el => {
    el.classList.remove("ship-preview", "ship-invalid");
  });
}
...
function canPlace(cells) {
  return cells.every(ci => state.myGrid[ci] === null);
}
...
function buildGameGrids() {
  buildGrid("my-grid", false);
  buildGrid("opponent-grid", true);
  renderMyGrid();
  renderOpponentGrid();
}
...
function buildWeaponList() {
  ...
  list.innerHTML = `
    <div class="weapon-item normal-selected" data-weapon="normal" onclick="selectWeapon('normal')">
...
window.selectWeapon = function(id) {
  ...
  document.querySelectorAll(".weapon-item").forEach(el => {
    ...
    if (el.dataset.weapon === id) {
      el.classList.add(id === "normal" ? "normal-selected" : "selected");
    }
  });
};
...
function fireAtCell(i) {
  ...
  let targets = getTargetCells(i, weapon);
  targets = targets.filter(ci => state.opponentGrid[ci] === null);
...
function getTargetCells(mainIdx, weapon) {
  ...
  if (weapon === "random") {
    ...
    const available = [];
    for (let i = 0; i < 100; i++) {
      if (i !== mainIdx && state.opponentGrid[i] === null) available.push(i);
    }
    shuffle(available);
...
function getAtomicCells(mainIdx) {
  ...
  for (let r = 0; r < 10; r++) {
    for (let c = 0; c < 10; c++) {
      const dr = r - cr;
      const dc = c - cc;
      if ((dr * dr) / (radius * radius) + (dc * dc) / (radius * radius) <= 1) {
        cells.add(idx(r, c));
      }
    }
  }
...
function handleFireResult(data) {
  ...
  // Vérifier distribution d'arme (par tour joué par moi)
  state.turnCount++;
  state.nextWeaponIn--;
  if (state.nextWeaponIn <= 0) {
    const weaponKeys = Object.keys(WEAPONS_CONFIG);
    const given = weaponKeys[Math.floor(Math.random() * weaponKeys.length)];
    addWeapon(given);
    state.nextWeaponIn = rollNextWeaponDelay();
  }
}
...
function buildShipStatus() {
  const container = document.getElementById("my-ships-status");
  container.innerHTML = "";
  SHIPS_CONFIG.forEach(ship => {
    ...
    el.innerHTML = `
      <div class="ship-status-dot${sunk ? " sunk" : ""}"></div>
      <span>${ship.emoji} ${ship.name}</span>`;
...
document.querySelectorAll(".emoji-btn").forEach(btn => {
  btn.addEventListener("click", () => {
    ...
    publish("chat", { text: emoji });
    addChatMessage(state.pseudo, emoji, true);
  });
});
...
function sendChat() {
  ...
  if (!text) return;
  publish("chat", { text });
  addChatMessage(state.pseudo, text, true);
  input.value = "";
}
...
window.addEventListener("beforeunload", () => {
  if (state.channel) publish("disconnect", {});
});
